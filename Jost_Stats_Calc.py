...
modified by Patrick Reeves 1-26-2016 to correct deprecated use of scipy.stats
...
def mean_D_across_loci(results):
	...
	def approximate_hmean(dest_values):
		...
		A = numpy.mean(dest_values)                     #PR 1-26-2016
		#varD = stats.var(dest_values)
		varD = numpy.var(dest_values)                   #PR 1-26-2016
...
#calculate means for G'st and D by averaging Hs and Ht across loci #PR 1-26-2016
#loclist is results[1], the estimated stats per locus, statdesired is "G_Hedrick" or "D_Jost"
def mean_across_loci(loclist, statdesired, n): 
	
	Hs_est_list = []
	Ht_est_list = []
	G_est_list = [] 

	#add stats for each locus to each list, for Hs, Ht, and G
	for loc in loclist:
		s = loc[2]
		Hs_est_list.append(s)
		
		t = loc[3]
		Ht_est_list.append(t)
		
		#g = loc[4]
		#G_est_list.append(g)
		
	#calculate mean values
	mean_Hs_est = numpy.mean(Hs_est_list)
	mean_Ht_est = numpy.mean(Ht_est_list)
	mean_G_est = (mean_Ht_est-mean_Hs_est)/mean_Ht_est #this version used to avoid nan when taking mean on locus by locus Gst
	#mean_G_est = numpy.mean(G_est_list)	
	 
	#return the requested statistic
	if statdesired == "G_Hedrick":
		#calculate average G'st across loci
		#basic formula taken from above: G_Hedrick = (G_est*(n-1+Hs_est))/((n-1)*(1-Hs_est))
		#substituting mean values for each variable
		mean_G_Hedrick = (mean_G_est*(n-1+mean_Hs_est))/((n-1)*(1-mean_Hs_est))
		return mean_G_Hedrick
		
	elif statdesired == "D_Jost":
		#calculate average D across loci
		#basic formula taken from above: D_est = ((Ht_est-Hs_est)/(1-Hs_est))*(n/(n-1))
		#substituting mean values for each variable
		mean_D_Jost = ((mean_Ht_est-mean_Hs_est)/(1-mean_Hs_est))*(n/(n-1))
		return mean_D_Jost
